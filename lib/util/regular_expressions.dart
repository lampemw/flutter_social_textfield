...
/// Regular expression to extract workflow variables in the format {{ variable }}
final workflowVariableRegExp = RegExp(
  r'\{\{\s*([^}]+?)\s*\}\}',
  multiLine: true,
);

final workflowVariableStartRegExp = RegExp(
  r'\{\{ ?[^ ]* ?\}?}?',
  multiLine: true,
);

final workflowVariableEndRegExp = RegExp(
  r'\s*\}\}',
  multiLine: true,
);
